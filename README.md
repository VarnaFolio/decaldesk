...
Every product (Simple or Variable) also gets a **SKU**, generated by rule from the name, size, category, and material (e.g. `koleda-50x70-kitchen-matte`) — not by AI, so it's predictable, stable, and available even when AI is off. If the generated SKU collides with an existing product, a numeric suffix (`-2`, `-3`, ...) is appended automatically.
