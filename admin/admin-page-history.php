...
class DecalDesk_Jobs_List_Table extends WP_List_Table {
	...
	protected function column_status( $item ) {
		$badges = array(
			'pending'    => array(
				'label' => __( 'Pending', 'decaldesk' ),
				'color' => '#dba617',
				'bg'    => '#fcf9e8',
			),
			'processing' => array(
				'label' => __( 'Processing', 'decaldesk' ),
				'color' => '#2271b1',
				'bg'    => '#eef6fc',
			),
			'done'       => array(
				'label' => __( 'Done', 'decaldesk' ),
				'color' => '#00a32a',
				'bg'    => '#edfaef',
			),
			'error'      => array(
				'label' => __( 'Error', 'decaldesk' ),
				'color' => '#d63638',
				'bg'    => '#fcf0f1',
			),
		);
		...
		$status = $item['status'];
		$badge  = isset( $badges[ $status ] ) ? $badges[ $status ] : array(
			'label' => $status,
			'color' => '#646970',
			'bg'    => '#f0f0f1',
		);
	...
	protected function column_ai_source( $item ) {
		...
		$badges = array(
			'ai_free'   => array(
				'label' => __( 'AI (free)', 'decaldesk' ),
				'color' => '#007a2e',
				'bg'    => '#e7f5ea',
			),
			'ai_claude' => array(
				'label' => __( 'AI (Claude)', 'decaldesk' ),
				'color' => '#6b21a8',
				'bg'    => '#f1e9fb',
			),
			'fallback'  => array(
				'label' => __( 'Template', 'decaldesk' ),
				'color' => '#646970',
				'bg'    => '#f0f0f1',
			),
		);
	...
	protected function process_bulk_action() {
		...
		if ( 'publish' === $this->current_action() ) {
			...
			add_action(
				'admin_notices',
				function () use ( $published, $skipped ) {
					$message = sprintf(
